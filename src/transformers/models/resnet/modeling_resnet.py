...
class ResNetForImageClassification(ResNetPreTrainedModel):
    ...
    def forward(
        ...
        # 处理 original_images
        original_outputs = self.resnet(original_images, output_hidden_states=output_hidden_states,
                                       return_dict=return_dict)
        original_pooled = original_outputs.pooler_output if return_dict else original_outputs[1]
        logits1 = self.classifier1(original_pooled)

        # 处理 sub_images
        sub_outputs = self.resnet(sub_images, output_hidden_states=output_hidden_states, return_dict=return_dict)
        sub_pooled = sub_outputs.pooler_output if return_dict else sub_outputs[1]
        logits2 = self.classifier2(sub_pooled)
        y = None
        if self.use_diff:
            # diffsort_logits
            x = sub_pooled.squeeze()
            # 通过线性层传递 x
            x = F.relu(self.fc1(x))
            x = self.fc2(x)
            y = x.squeeze(1)
