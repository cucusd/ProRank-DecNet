...
class SwiftFormerForImageClassification(SwiftFormerPreTrainedModel):
    def __init__(self, config: SwiftFormerConfig,num_labels1: int, num_labels2: int,use_diff: bool = False) -> None:
        super().__init__(config)
        ...
        self.head1 = nn.Linear(embed_dims[-1], self.num_labels1) if self.num_labels1 > 0 else nn.Identity()
        self.dist_head1 = nn.Linear(embed_dims[-1], self.num_labels1) if self.num_labels1 > 0 else nn.Identity()

        self.head2 = nn.Linear(embed_dims[-1], self.num_labels2) if self.num_labels2 > 0 else nn.Identity()
        self.dist_head2 = nn.Linear(embed_dims[-1], self.num_labels2) if self.num_labels2 > 0 else nn.Identity()

        # Initialize weights and apply final processing
        self.post_init()
        # 创建线性层并移动到指定设备
        self.fc1 = nn.Linear(220, 64)
    ...
    def forward(
        self,
        original_images: Optional[torch.FloatTensor] = None,
        sub_images: Optional[torch.FloatTensor] = None,
        pixel_values: Optional[torch.Tensor] = None,
        ...
        #
        # # tensor = sequence_output.view(-1, 3, sequence_output.shape[1], sequence_output.shape[2], sequence_output.shape[3])
        # # sequence_output = torch.max(tensor, dim=1)[0]
        #
        ...
        # # 处理 original_images
        original_outputs = self.swiftformer(
            original_images,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        )

        original_sequence_output = original_outputs.last_hidden_state if return_dict else original_outputs[0]

        # run classification head
        original_sequence_output = self.norm(original_sequence_output)
        original_sequence_output = original_sequence_output.flatten(2).mean(-1)
        cls_out = self.head1(original_sequence_output)

        distillation_out = self.dist_head1(original_sequence_output)
        logits1 = (cls_out + distillation_out) / 2

        # 处理 sub_images
        sub_outputs = self.swiftformer(
            sub_images,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        )

        sub_sequence_output = sub_outputs.last_hidden_state if return_dict else sub_outputs[0]

        # run classification head
        sub_sequence_output = self.norm(sub_sequence_output)
        sub_sequence_output = sub_sequence_output.flatten(2).mean(-1)
        cls_out = self.head2(sub_sequence_output)

        distillation_out = self.dist_head2(sub_sequence_output)
        logits2 = (cls_out + distillation_out) / 2

        y = None
        if self.use_diff:
            # diffsort_logits
            x = sub_sequence_output
            ...
            y = x.squeeze(1)
            y = y.unsqueeze(0)
            # tensor = logits.view(-1, 3, 3)
            # logits = torch.max(tensor, dim=1)[0]
        ...
        return ImageClassifierOutputWithNoAttention(loss=loss,logits1=logits1,logits2=logits2,logits_diff = y)
