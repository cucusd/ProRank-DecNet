...
class ConvNextV2ForImageClassification(ConvNextV2PreTrainedModel):
    def __init__(self, config,num_labels1: int, num_labels2: int,use_diff: bool = False):
        super().__init__(config)
        ...
        self.classifier1 = (
            nn.Linear(config.hidden_sizes[-1], num_labels1) if num_labels1 > 0 else nn.Identity()
        )
        self.classifier2 = (
            nn.Linear(config.hidden_sizes[-1], num_labels2) if num_labels2 > 0 else nn.Identity()
        )
    ...
    def forward(
        ...
        # 处理 original_images
        original_outputs = self.convnextv2(original_images, output_hidden_states=output_hidden_states, return_dict=return_dict)
        original_pooled_output = original_outputs.pooler_output if return_dict else original_outputs[1]
        logits1 = self.classifier1(original_pooled_output)
        # 处理 sub_images
        sub_outputs = self.convnextv2(sub_images, output_hidden_states=output_hidden_states, return_dict=return_dict)
        sub_pooled_output = sub_outputs.pooler_output if return_dict else sub_outputs[1]
        logits2 = self.classifier2(sub_pooled_output)
        # tensor = pooled_output.view(-1, 3, pooled_output.shape[1])
        # pooled_output = torch.max(tensor, dim=1)[0]
        y = None
        if self.use_diff:
            # diffsort_logits
            x = sub_pooled_output
        ...
        # len = len(labels)
        # tensor = logits.view(len(labels),-1, 3)
        # logits = torch.max(tensor, dim=1)[0]
