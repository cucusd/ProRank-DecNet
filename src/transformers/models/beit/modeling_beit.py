...
class BeitForImageClassification(BeitPreTrainedModel):
    def __init__(self, config: BeitConfig,num_labels1: int, num_labels2: int,use_diff: bool = False) -> None:
        super().__init__(config)
        ...
        self.classifier1 = nn.Linear(config.hidden_size,  num_labels1) if  num_labels1 > 0 else nn.Identity()
        self.classifier2 = nn.Linear(config.hidden_size, num_labels2) if num_labels2 > 0 else nn.Identity()
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        return ImageClassifierOutput(loss=loss,logits1=logits1,logits2=logits2,logits_diff = y)
