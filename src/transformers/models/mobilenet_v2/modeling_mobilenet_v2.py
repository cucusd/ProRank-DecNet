...
class MobileNetV2ForImageClassification(MobileNetV2PreTrainedModel):
    def __init__(self, config: MobileNetV2Config,num_labels1: int, num_labels2: int,use_diff: bool = False) -> None:
        super().__init__(config)
        ...
        self.classifier1 = nn.Linear(last_hidden_size, num_labels1) if num_labels1 > 0 else nn.Identity()
        self.classifier2 = nn.Linear(last_hidden_size, num_labels2) if num_labels2 > 0 else nn.Identity()

        # Initialize weights and apply final processing
        self.post_init()
        self.fc1 = nn.Linear(self.mobilenet_v2.conv_1x1.convolution.out_channels, 64)
    ...
    def forward(
        ...
        # 处理 original_images
        original_outputs = self.mobilenet_v2(original_images, output_hidden_states=output_hidden_states, return_dict=return_dict)
        original_pooled_output = original_outputs.pooler_output if return_dict else original_outputs[1]
        logits1 = self.classifier1(self.dropout(original_pooled_output))
        # 处理 sub_images
        sub_outputs = self.mobilenet_v2(sub_images, output_hidden_states=output_hidden_states, return_dict=return_dict)
        sub_pooled_output = sub_outputs.pooler_output if return_dict else sub_outputs[1]
        logits2 = self.classifier2(self.dropout(sub_pooled_output))
        ...
        return ImageClassifierOutputWithNoAttention(
            loss=loss, logits1=logits1, logits2=logits2, logits_diff=y
        )
        # return ImageClassifierOutputWithNoAttention(logits=logits, logits_diff=y, hidden_states=outputs.hidden_states)
