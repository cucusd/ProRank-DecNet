...
class SwinImageClassifierOutput(ModelOutput):
    ...
    logits_diff: torch.FloatTensor = None
...
class SwinForImageClassification(SwinPreTrainedModel):
    def __init__(self, config,num_labels1: int, num_labels2: int,use_diff: bool = False):
        ...
        # )
        self.classifier1 = (
            nn.Linear(self.swin.num_features, num_labels1) if num_labels1 > 0 else nn.Identity()
        )
        self.classifier2 = (
            nn.Linear(self.swin.num_features, num_labels2) if num_labels2> 0 else nn.Identity()
        )
    ...
    def forward(
        ...
        original_pooled_output = original_outputs[1]
        logits1 = self.classifier1(original_pooled_output)
        # 处理 sub_images
        sub_outputs = self.swin(
            sub_images,
            head_mask=head_mask,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
        ...
        sub_pooled_output = sub_outputs[1]
        ...
        return SwinImageClassifierOutput(loss=loss, logits1=logits1, logits2=logits2, logits_diff=y)
