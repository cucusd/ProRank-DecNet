...
def main(config):
    ...
    #         "original_indices": original_indices,  # 关键索引映射
    #         "sample_ids": sample_ids  # 样本唯一标识
    #     }
    ...
    def aggregate_sub_predictions(sub_logits, original_indices):
        """
        统计每个原图中：
        - 第0列：预测为类别0的子图概率和
        - 第1列：预测为类别1和类别2的子图概率和（合并）

        参数:
            sub_logits: [N, 3]          # 子图的三分类logits
            original_indices: [N]       # 子图对应的原图索引

        返回:
            prob_counts: [B, 2]         # 原图的概率统计:
                                        # - 第0列：类别0的概率和
                                        # - 第1列：类别1+2的概率和
        """
        # 计算子图类别概率和预测类别 [N, 3]
        sub_probs = F.softmax(sub_logits, dim=-1)
        sub_preds = torch.argmax(sub_logits, dim=1)  # 实际预测类别

        B = torch.max(original_indices).item() + 1
        device = sub_logits.device
        prob_counts = torch.zeros(B, 2, device=device)  # 改为2列

        for idx in torch.unique(original_indices):
            mask = original_indices == idx
            probs_group = sub_logits[mask]  # [k, 3]
            preds_group = sub_preds[mask]  # [k]

            # 统计预测为类别0的概率和
            class0_mask = preds_group == 0
            prob_counts[idx, 0] = probs_group[class0_mask, 0].sum()

            # 统计预测为类别1和2的概率和（合并）
            class1_mask = preds_group == 1
            class2_mask = preds_group == 2
            prob_counts[idx, 1] = (
                    probs_group[class1_mask, 1].sum() +
                    probs_group[class2_mask, 2].sum()
            )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                # counts = aggregate_sub_predictions(outputs.logits2, batch['original_indices'])
                # loss_con_2 = loss_fct(counts.view(-1, args.num_labels), batch['original_labels'].view(-1))




                # aggregator = DifferentiableAggregation(k_init=1.0, k_max=10.0, growth='exponential')
                # aggregator = DifferentiableAggregation_test(k=args.k)
                # P_original_2d = aggregator(outputs.logits2,batch['original_indices'])
                ...
                # loss_con = loss_fct(logits3.view(-1, args.num_labels), batch['original_labels'].view(-1))
                T =args.t
                soft_targets = F.softmax(outputs.logits1 / T, dim=-1)
                # P_log_probs = torch.log(logits3)  # 计算对数概率
                # targets_log_probs = torch.log(soft_targets)  # 计算对数概率
                # loss_fn = NLLLoss()
                # loss_con = loss_fn(P_log_probs, targets_log_probs)  # 等价于CrossEntropyLoss的原始逻辑
                loss_kl = F.kl_div(F.log_softmax(logits3 / T, dim=-1), soft_targets,reduction='batchmean')


                loss = loss1 +  loss2 + 0.3 * loss_kl
                # loss = 0.4 * loss1 + 0.4 * loss2 + 0.2 * loss_con  # 显式加权
        ...
        if accelerator.is_main_process:
            epoch_model_path = os.path.join(args.output_dir, f"epoch_{epoch}")
...
if __name__ == '__main__':
    ...
    num_labels = 2
    num_labels_sub = 3
    # model_name = ['resnet50', 'mobilenet_v2', 'convnextv2-tiny', 'swin', 'deit', 'beit', 'swiftformer-xs', 'vit-base']
    model_name = ['swin']
    # model_name = ['deit','beit','swiftformer-xs','vit-base']
    # model_name = ['vit-base']
    ...
    for jj in range(len(model_name)):
        ...
        #     args.train_dir = '/data1/jingyi/cjy/data/huggingface/guizhou_feiqu_complete/filename_txt/dataset_splits_year_8ka/train.txt'
        #     args.validation_dir = '/data1/jingyi/cjy/data/huggingface/guizhou_feiqu_complete/filename_txt/dataset_splits_year_8ka/val.txt'
        ...
        #     args.output_dir = f'/data1/jingyi/cjy/pyproject/transformers/results/year_sub_ori/two_module_true/cls3_con_03_k_1_t_{i}/{args.model_name}'
        #
        #     main(args)
        ...
        for k in [3]:
            ...
            # args.output_dir = model_name
            # if model_name == 'resnet50':
            args.output_dir = f'output/shanxi_train_old_data_Stage1x3_prama_BRCD_mix/{args.model_name}'+'-k'+str(args.k)
            main(args)
            test_function(args.output_dir)
